...
| F4 | Profiling-driven speed | ACTIVE (C2 shipped) | C2 done and merged (I010/I011): bit-identical, fit −4.0% okcupid-stem, −6.8% kick. Next = C1 (`_softmax`), whose S0 owes a wall-clock read before any forecast |
...
next: **C2 SHIPPED** 2026-08-16 (I010 S0, I011 S1) — `_factorize_hashed`, bit-identical, 3–6% off fit on string-categorical sets, nothing elsewhere; the family has produced a win. C1 (`_softmax`, multiclass only) is the next unit, and its S0 owes a WALL-CLOCK measurement of the softmax leg before any forecast — C2 proved a cProfile share on a Python loop overstates (18% profiled, 8% real). C1's is numpy calls rather than a loop, so it may hold up, but it is not to be forecast off the profile alone.
