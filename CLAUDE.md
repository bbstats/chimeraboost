...
- **Sealed holdouts are report-only:** TabArena (Lite and Full) and the `pub:` public suite (`benchmarks/PUBLIC_PLAN.md`, behind the published chart). Their results — aggregate or per-task — must never influence a source change. Decisions run on synthetic → dev panel → Grinsztajn + high-card, gated by an independent OpenML one-shot. PMLB is the HP-tuning suite only. The published chart must never run on a suite we tune against; that would be in-sample.
