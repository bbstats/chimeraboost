...
| `audition` | `True` | Fit three candidates and keep the one with the best CRPS on the rows early stopping held out: the head as configured, the same head with 254 histogram bins, and the head's shape moved onto the median of a squared-error `ChimeraBoostRegressor`. Every prediction method and `shap_values` serve the winner, recorded in `audition_`. About 2.35 times the fit time of one head. It needs early-stopping rows and `conformalize="auto"`; otherwise one head is fitted. `False` fits one head, as in earlier releases. |
